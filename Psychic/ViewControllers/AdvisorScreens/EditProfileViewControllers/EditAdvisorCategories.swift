...
class EditAdvisorCategories: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UINavigationControllerDelegate {
            ...
            Alamofire.request(baseurl, method: .get, parameters: parameters, encoding: URLEncoding.default).responseJSON{ (responseData) -> Void in
                if((responseData.result.value) != nil)
                {
                    ...
                    for i in 0..<self.categories.count
                    {
                        ...
                        let oneCat = self.categories[i]
                        let id = oneCat.getID()
                        
                        for j in 0..<ADV_CATEGORIES.count
                        {
                            let oneAD = ADV_CATEGORIES[j]
                            let selecteID = "\(oneAD["id"])"
                            
                            if selecteID == id
                            {
                                self.selectedIDs.append(id)
                                oneCat.setStatus(value: "1")
    ...
    func assignCategories()
    {
        if Reach.isConnectedToNetwork()
        {
            ...
            NEW_CATEGORIES = stringIds
            
            self.performSegue(withIdentifier: "decs", sender: nil)
